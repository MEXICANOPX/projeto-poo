...
function displayTasks() {
  ...
  tasks.forEach((task, index) => {
    ...
    let taskInfo = `${index + 1}. ${task.description} (${status})`;

    if (task instanceof TarefaRepetitiva) {
      taskInfo += ` - Repetitiva (${task.frequency}, Início: ${task.startDate})`;
    } else if (task instanceof TarefaPrioritária) {
      taskInfo += ` - Prioritária (Prioridade: ${task.priority}, Deadline: ${task.deadline})`;
    } else if (task instanceof TarefaComEtiqueta) {
      taskInfo += ` - Etiqueta (Etiquetas: ${task.tags.join(', ')})`;
    }
